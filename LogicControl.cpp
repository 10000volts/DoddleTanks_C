...
void RemoveButton(Button * ls)
{
	...
	for (i = 0; i < buttonCount_; ++i) {
		if (m_buttonManager_ + i == ls) {
			memcpy(m_buttonManager_ + i, m_buttonManager_ + i + 1, buttonCount_ - i - 1);
			--buttonCount_;
		}
	}
}
...
void StepCheckFocus(LogicStep* tis) {
	int i;
	short mx = g_mouseState_.x, my = g_mouseState_.y;
	LogicSprite* ls;
	for (i = 0; i < buttonCount_; ++i) {
		ls = m_buttonManager_[i].ls_;
		if (IsIn(mx, my, ls->m_x_, ls->m_y_, ls->m_w_, ls->m_h_)) {
			if (m_focusButton_ != NULL) {
				m_focusButton_->OnLeave(m_focusButton_);
			}
			m_focusButton_->OnFocus(m_focusButton_);
			m_focusButton_ = &m_buttonManager_[i];
			if (g_mouseState_.mkLButton) {
